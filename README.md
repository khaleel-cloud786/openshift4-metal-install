# OpenShift 4 Bare Metal Install (VMware) - User Provisioned Infrastructure (UPI) - Installation
...
1. Download [config files](https://github.com/khaleel-cloud786/openshift4-metal-install) for each of the services

   ```bash
   git clone https://github.com/khaleel-cloud786/openshift4-metal-install
   ```
...
1. Install and configure BIND DNS
   ...
   ```bash
   \cp ~/openshift4-metal-install/dns/named.conf /etc/named.conf
   cp -R ~/openshift4-metal-install/dns/zones /etc/named/
   ```
   ...
   > At the moment DNS will still be pointing to the LAN DNS server. You can see this by testing with `dig ocp.shaik`.
   ...
   ```bash
   dig ocp.shaik
   # The following should return the answer ocp-bootstrap.khaleel.ocp.shaik from the local server
...
1. Install & configure DHCP
   ...
   ```bash
   \cp ~/openshift4-metal-install/dhcpd.conf /etc/dhcp/dhcpd.conf
   ```
...
1. Install & configure HAProxy
   ...
   ```bash
   \cp ~/openshift4-metal-install/haproxy.cfg /etc/haproxy/haproxy.cfg
   ```
...
1. Copy the install-config.yaml included in the clones repository to the install directory
   ...
   ```bash
   cp ~/openshift4-metal-install/install-config.yaml ~/ocp-install
   ```
...
1. Create the persistent volume for the 'image-registry-storage' pvc to bind to
   ...
   ```bash
   oc create -f ~/openshift4-metal-install/manifest/registry-pv.yaml
   ```
...
1. Apply the `oauth-htpasswd.yaml` file to the cluster
   ...
   ```bash
   oc apply -f ~/openshift4-metal-install/manifest/oauth-htpasswd.yaml
   ```
...
1. Append the following to your local workstations `/etc/hosts` file:
   ...
   > If you do not want to add an entry for each new service made available on OpenShift you can configure the ocp-svc DNS server to serve externally and create a wildcard entry for \*.apps.khaleel.ocp.shaik
   ...
   192.168.0.96 ocp-svc api.khaleel.ocp.shaik console-openshift-console.apps.khaleel.ocp.shaik oauth-openshift.apps.khaleel.ocp.shaik downloads-openshift-console.apps.khaleel.ocp.shaik alertmanager-main-openshift-monitoring.apps.khaleel.ocp.shaik grafana-openshift-monitoring.apps.khaleel.ocp.shaik prometheus-k8s-openshift-monitoring.apps.khaleel.ocp.shaik thanos-querier-openshift-monitoring.apps.khaleel.ocp.shaik
   ```

1. Navigate to the [OpenShift Console URL](https://console-openshift-console.apps.khaleel.ocp.shaik) and log in as the 'admin' user
